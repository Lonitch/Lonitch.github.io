...
Now we use the information above to derive a secular relation for calculating $$E_{n\vec{k}}$$. Assume that we know $$ u_{n \vec{k}}(r) $$ and $$E_{n\vec{k}}$$ for all "$n$" at $$\vec{k}_0$$, then we can expand $$u_{n\vec{k}}$$ at $$\vec{k}$$ in terms of $$u_{n\vec{k_0}}$$ as
...
The "**secular equation**" for $E_{n\vec{k}}$ follows from the condition for the existence of nontrivial solutions to this set of homogeneous linear equations,
...
- If only an under-complete set of $$u_{n\vec{k}_0}$$ is used, the k-p method is accurate only for $\vec{k}$ near $\vec{k}_0$.
- When sufficiently large set of $$u_{n\vec{k}_0}$$ is used, the band structure in FBZ can be approximated.
